...
# ================================================================
# 【往期优化内容 (vpsge-v9)】
#   1. 品牌升级：脚本全面升级为 vpsge-v9，快捷命令也同步修改为 vpsge。
#   2. Realm 集成：将您提供的 Realm 端口转发脚本完美模块化，并放置于
#      「三、安装服务」与「五、服务管理」子菜单中，保持了 UI 的高度统一。
#   3. Realm 逻辑修复：修复了原版 sc.sh 在「删除转发规则」时，行号计算
...
#
# 【本次优化内容 (vpsge-v10)】
#   1. 彻底解决“幽灵依赖”与“状态误判”：引入全路径强力探测引擎 is_cmd_exist()，
#      无视 Linux 环境变量缺失问题，精准分离“未安装”、“已停止”与“运行中”状态。
#   2. 面板访问被拦截修复：针对 Nginx 1.27+ 废弃 http2 参数导致致命报错崩溃的问题，
#      安全回退配置格式；新增 iptables 原生兜底放行逻辑，无视 UFW/Firewalld 状态强制打通 8443/8080。
#   3. Nginx 配置文件自动恢复：重构包管理器逻辑，针对 apt 引入 --force-confmiss 参数，
#      彻底解决卸载重装后报错 nginx.conf 丢失导致连锁断流罢工的“顽疾”。
#   4. sing-box 全自动守护运行：在配置节点生成且语法校验通过后，自动将 sing-box
#      加入 systemd 守护进程并拉起运行，无需再退回服务管理菜单手动开启。
#   5. 版本全量平滑迁移：核心升级为 vpsge-v10，脚本直链与更新命令同步迭代。
# ================================================================
...
#  全局变量 & 直链配置
# ────────────────────────────────────────────────────────────────
VPSGE_REMOTE_URL="https://raw.githubusercontent.com/github19999/Ojddj/main/vpsge-v10.sh"
SCRIPT_VERSION="vpsge-v10"
...
menu_manage_substore() {
    while true; do
        ...
        case $opt in
            1) 
                if is_cmd_exist docker && docker ps -a --format '{{.Names}}' | grep -q "^substore$"; then
...
menu_manage_wallos() {
    while true; do
        ...
        case $opt in
            1) 
                if is_cmd_exist docker && docker ps -a --format '{{.Names}}' | grep -q "^wallos$"; then
