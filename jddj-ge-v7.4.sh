...
#   优化1：修复导入旧节点时，TUIC、Naïve 等协议密码若包含特殊字符 (如 ^, @, ?)
#         被 URL Encode 导致提取失败、截断断层的 BUG。提前加入 URL Decode 处理。
#   优化2：增加自动加载本地历史节点配置机制。退出脚本后重新进入时，能自动识别 
#         /etc/sing-box/subscription.txt 并提供一键导入，省去反复粘贴的烦恼。
# ================================================================
# 【往期核心优化回顾】
