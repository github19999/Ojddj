...
# ================================================================
#   服务器一键管理脚本 (jddj)
#   版本号：jddj-ge-v7.3 (优化版)
#   集成：SSH安全加固 / SSL证书 / sing-box 安装配置 / 节点生成
# ================================================================
# 【本次优化内容 (jddj-ge-v7.3 优化版)】
#   优化1：彻底修复 VLESS-REALITY 在旧节点导入或静默生成时失效的问题。
#         - 放弃了 Python 的决定性密钥推导算法（兼容性差），改用 sing-box 
#           原生命令生成完全可靠的随机密钥。
#         - 采纳“Tag 藏钥法”，将 private_key 隐藏在节点分享链接的 tag 
#           (备注) 中。导入时自动解析提取，完美实现 REALITY 节点的无损重装。
#         - 在生成 Clash 订阅时会自动美化名称（隐藏私钥后缀）。
#   优化2：修复了导入 TUIC 等包含特殊字符（如 @、? 等）密码的旧节点时，
#         Python 解析 URL 失败导致参数丢失的严重 BUG。
#         - 弃用标准 urllib 解析，重写了底层 URL 逆向切割逻辑，确保
#           任意复杂密码都能精准提取和导入。
# ================================================================
    ...
    while true; do
        ...
        if [[ "$import_choice" == "1" ]]; then
            ...
            if [[ "$has_input" == "true" ]]; then
                ...
                # Python 解析脚本（彻底重构底层 URL 逆向切割，完美支持特殊字符密码）
                cat > "$py_script" << 'PYEOF'
...
for line in input_text.splitlines():
    ...
    try:
        if line.startswith("vmess://"):
            ...
            # 安全重构版的 URL 切割逻辑，完美规避密码含 @ 或 ? 导致的截断
...
if not vars_out:
    ...
    for k, v in vars_out.items():
        # 强转义机制替换特殊符号，防止环境变量在 bash 传承中被吞噬
        v_escaped = str(v).replace("'", "'\\''")
        print(f"export {k}='{v_escaped}'")
    print("echo -e \"\\033[0;32m[✓] 解析完成，已成功提取匹配节点的参数（涵盖端口、特殊密码、私钥、UUID 等）。\\033[0m\";")
PYEOF
                local parse_exports
...
for ib in inbounds:
    ...
    if t == 'vless':
        ...
        # 针对 Clash YAML 显示进行 tag 美化（隐藏 Reality Tag 中附带的私钥）
        display_tag = tag
        if reality_on and re.search(r'-[A-Za-z0-9_-]+$', tag):
            display_tag = tag.rsplit('-', 1)[0]
            
        # 注意：分享链接保留了完整的 tag (含私钥)，以便完美无损重装！
