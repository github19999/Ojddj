...
deploy_ssl() {
    ...
    local packages=""
    [[ "$PKG_MANAGER" == "apt" ]] && \
        packages="curl wget socat cron openssl ca-certificates git" || \
        packages="curl wget socat cronie openssl ca-certificates git"
    ...
    # ── 安装 acme.sh ───────────────────────────────────────────────
    # 彻底弃用基于 crontab 校验的在线脚本，改用 git 拉取后直接 --force 绕过环境预检安装
    if [[ ! -f /root/.acme.sh/acme.sh ]]; then
        ...
        if git clone https://github.com/acmesh-official/acme.sh.git /tmp/acme_sh_install >/dev/null 2>&1; then
            ...
            # 若 git 失败，降级采用 curl 下载源码包进行强制安装
