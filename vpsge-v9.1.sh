...
#修复细节说明：
#优化1（卸载后仍提示已安装）：
#原因：Linux 系统的 Bash 环境存在命令缓存（Hash Cache），且有些包管理器卸载后可能会在其他目录（如 /usr/local/bin/ 等）残留二进制文件，导致判断语句 command -v 发生误判。
#修复：重写了卸载函数，加入了遍历并强制删除系统内所有名为 sing-box 和 nginx 文件的逻辑，并在检测安装前强制刷新 Bash 缓存 (hash -r)，同时加入真实文件存在性检测 [ -f "$(command -v ...)" ]，彻底解决“幽灵依赖”问题。

#优化2（Sub-Store/Wallos 面板无法访问）：
#原因 1（Nginx 致命报错阻断反代）：在最新版的 Nginx 1.27+ 中，listen 8443 ssl http2; 语法中的 http2 参数已经被官方彻底移除。如果使用此语法，Nginx 会抛出致命错误 ([emerg] invalid parameter "http2") 导致无法启动，反代环境随之瘫痪。我已将配置文件中的监听语法回退为高兼容的 listen 8443 ssl;。
#原因 2（底层防火墙拦截）：旧逻辑仅放行了 UFW 和 Firewalld，但很多纯净版系统默认使用底层的 iptables 进行网络拦截。我在 open_firewall_ports 函数中补充了无视上层状态、直接写入 iptables / ip6tables 放行 8443、8080 等面板端口的兜底逻辑。
