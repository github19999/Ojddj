...
gen_reality_keypair() {
    local seed_uuid="$1"
    if [[ -z "$seed_uuid" ]]; then
        seed_uuid=$(cat /proc/sys/kernel/random/uuid 2>/dev/null || python3 -c "import uuid; print(uuid.uuid4())")
    fi
...
def x25519_base(k_bytes):
    ...
    for i in range(254, -1, -1):
        bit = (k >> i) & 1
        if bit:
            x_3 = (x_1*x_2 - z_1*z_2)**2 % P
            z_3 = ((x_1*z_2 - z_1*x_2)**2 * 9) % P
            x_4 = (x_2**2 - z_2**2)**2 % P
            z_4 = (4*x_2*z_2*(x_2**2 + A24*x_2*z_2 + z_2**2)) % P
            x_1, z_1, x_2, z_2 = x_3, z_3, x_4, z_4
        else:
            x_3 = (x_1*x_2 - z_1*z_2)**2 % P
            z_3 = ((x_1*z_2 - z_1*x_2)**2 * 9) % P
            x_4 = (x_1**2 - z_1**2)**2 % P
            z_4 = (4*x_1*z_1*(x_1**2 + A24*x_1*z_1 + z_1**2)) % P
            x_2, z_2, x_1, z_1 = x_3, z_3, x_4, z_4
    return (x_1 * pow(z_1, P - 2, P)) % P

h = bytearray(hashlib.sha256(b'$seed_uuid').digest())
h[0] &= 248
h[31] &= 127
h[31] |= 64
priv_bytes = bytes(h)
pub_int = x25519_base(priv_bytes)
pub_bytes = pub_int.to_bytes(32, 'little')

print('PrivateKey: ' + base64.urlsafe_b64encode(priv_bytes).decode().rstrip('='))
print('PublicKey:  ' + base64.urlsafe_b64encode(pub_bytes).decode().rstrip('='))
"
}
...
build_vless_reality() {
    ...
    ask_random uuid "uuid（用户 UUID，作为密钥对生成的种子）" "${OLD_VLESS_REALITY_UUID:-$(gen_uuid)}"

    echo -e "  ${YELLOW}正在通过 UUID 决定性推导 REALITY 密钥对...${NC}"
    ...
    if [[ -n "$existing_pk" && -n "$existing_pub" ]]; then
        ...
        if [[ -n "$OLD_VLESS_REALITY_PBK" ]]; then
            if [[ "$OLD_VLESS_REALITY_PBK" == "$pubkey" ]]; then
                log_success "推导成功！基于 UUID 生成的新公钥与你的旧公钥完美匹配！"
            else
                log_warn "你的旧节点由不支持决定性推导的版本生成（随机私钥重装后已物理丢失）。"
                log_warn "【重要】已为您重新生成了绑定 UUID 的新密钥对，请务必在客户端更新本次生成的 Public Key！"
                log_info "（未来重装只要导入此 UUID，密钥对将自动 100% 还原，不再丢失）"
            fi
        fi
    fi
    ...
    if [[ "$AUTO_DEFAULT" == "true" ]]; then
        ...
        echo -e "  ${CYAN}◆ REALITY 密钥对（已基于 UUID 绑定生成，回车直接使用）${NC}"
...
for s in schemes:
    if s not in urllib.parse.uses_netloc:
        urllib.parse.uses_netloc.append(s)
...
PYEOF
        ...
        echo "   3)  VLESS — gRPC"
        echo "   4)  VLESS — REALITY (TCP + XTLS-Vision) [决定性推导绑定 UUID]"
