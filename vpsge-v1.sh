...
deploy_ssl() {
    ...
    # 根据系统启动 cron，修复原本的 systemctl enable 混用问题
    ...
    # 新版：检测并管理 Web 服务，为 80 端口释放腾出空间
    log_step "检测并管理 Web 服务..."
    ...
    if [[ -n "$port_info" ]]; then
        ...
        if [[ ${#detected_services[@]} -gt 0 ]]; then
            echo -e "${YELLOW}发现运行中的 Web 服务: ${detected_services[*]}${NC}"
            ...
            log_warn "端口 80 被占用，但未找到已知 Web 服务，尝试强制关闭占用 80 的进程"
            fuser -k 80/tcp 2>/dev/null || true
        fi
    else
        # 即使端口没被占用，也记录下哪些 web 服务处于 active 状态，后续好写 hook
        for service in "${web_services[@]}"; do
            if systemctl is-active --quiet "$service" 2>/dev/null; then
                detected_services+=("$service")
                systemctl stop "$service" 2>/dev/null || true
    ...
    # 写入 Pre/Post Hook 到 acme.sh 域名配置文件，彻底解决重载占用问题
    if [[ -n "$pre_hook" ]]; then
        ...
        if [[ -f "$conf_file" ]]; then
            if ! grep -q "Le_PreHook" "$conf_file"; then
                ...
                log_success "底层续期 Hook 配置完成: 续期时将自动停启 $detected_service_for_hook"
            else
                log_info "底层续期 Hook 已存在，跳过写入"
            fi
        else
            log_warn "未找到 acme.sh 配置文件: $conf_file，Hook配置失败"
        fi
    fi
    ...
    if ! crontab -l 2>/dev/null | grep -q "acme.sh.*--cron"; then
        ...
        # 检查是否静默丢弃日志，如果是则替换为输出日志的版本
        if crontab -l 2>/dev/null | grep "acme.sh.*--cron" | grep -q "/dev/null"; then
            log_warn "检测到旧版续期任务日志被丢弃，已自动更新 crontab 以记录日志"
            (crontab -l 2>/dev/null | grep -v "acme.sh.*--cron"; echo "$CRON_JOB") | crontab - 2>/dev/null
        fi
    fi
